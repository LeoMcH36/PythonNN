...
#current issue trying to overwrite model

#i can keep the first run stuff, but i need another run for each time new data is inputted that uses the weights were the previous data ended
...
def forwardPassTest(paragraph,label,model):
    ...
    featuresTest =  [averageWordLength(paragraph),mostCommonVowel(paragraph),leastCommonVowel(paragraph),doubleLettersFrequency(paragraph)]
    print(featuresTest)        
        ...
        if self.model == 0:
            for n in neurons:
                if n.name == self.nextNeuronName:
                    #print("next neuron name is ", n.name)
                    #print("weight ", self.weight)
                    #print("error ", n.error)
                    #print("input ", self.input)
                    #print("learningRate ", learningRate)
                    self.weight = self.weight + (n.error * self.input * learningRate)
                    return self.weight 
        else:
...
class Neuron:
    ...
    def calculateError(self):
        if self.outputBool == 1:
            ...
            count+=1
            #print("last",count )
           # print("outputf", self.value)
            #print("desired",desiredOutput)
            
            self.error = self.value * (1-self.value) * (self.desiredOutput - self.value)
            return self.error
        else:
            refN = ""
            error = 0
...
#def forwardPassTrain(inputA,inputB):
    
 #   arrow1 = arrow(0.1,inputA,"","1")


   # arrow2 = arrow(0.8,inputB,"","1")


   # neuron1 = Neuron("1",0)


   # arrow3 = arrow(0.4,inputA,"","2")
   # arrow4 = arrow(0.6,inputB,"","2")

   # neuron2 = Neuron("2",0)

   # print(neuron1.getOutput())
   # print(neuron2.getOutput())

   # arrow5 = arrow(0.3,neuron1.getOutput(),"1","3")
   # arrow6 = arrow(0.9,neuron2.getOutput(),"2","3")

   # neuron3 = Neuron("3",1)
   # print(neuron3.getOutput())
...
#need to do proper feature extraction, addition might be too difficult
...
for rowNum in range(len(dataLanguage)-1):
    ...
    features.append([[averageWordLength(paragraph),mostCommonVowel(paragraph),leastCommonVowel(paragraph),doubleLettersFrequency(paragraph)],dataLanguage[rowNum]['label']])
        

#

modelTemp = []
#each of the first 4 arrows are a feature?
firstRunFlag = True
for i in range(len(features)):
    ...
    for x in range(5000):
        ...
        if firstRunFlag :    
            ...
            arrowhidden1 = arrow(0.3,neuronFeature1.getOutput(),"nf1","h1",0)
            arrowhidden2= arrow(0.3,neuronFeature2.getOutput(),"nf2","h1",0)
            neuronHidden1 = Neuron("h1",0,0,features[i][1])
            
            #input neurons 3-4 connect to hidden 2
            arrowhidden3 = arrow(0.3,neuronFeature3.getOutput(),"nf3","h2",0)
            arrowhidden4 = arrow(0.3,neuronFeature4.getOutput(),"nf4","h2",0)
            ...
            arrowOutput1 = arrow(0.3,neuronHidden1.getOutput(),"h1","output",0)
            arrowOutput2 = arrow(0.3,neuronHidden2.getOutput(),"h2","output",0)
            ...
            #print( arrows[0].input)
            #print("###################################",modelTemp.arrows[0].input)
           # print("loop :", i)
            #print("feature not first", features[i][0][0])
            ...
            #modelTemp.neurons[0].getOutput()
            #modelTemp.neurons[1].getOutput()
            #print("run value")
            #modelTemp.neurons[2].getOutput()
    
        firstRunFlag = False
    
    
print("After Training")
#print(modelTemp.neurons[2].getOutput())   
...
for row in range(len(testData)):
    ...
    outcome = forwardPassTest(testData[row]['paragraph'],testData[row]['label'],modelLanguages)
...
#**********************************
#results
# learning rate 1, training loops 5000, outcome threshold 0.0024
#Accuracy = 68.75%